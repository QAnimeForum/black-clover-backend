...
@Injectable()
export class ItemNameEditWiazard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_NAME_ITEM_SCENE_ID,
            this.step1()
        );
    ...
    start() {
        return async (ctx: BotContext) => {
            await ctx.reply('Введите новое название предмета.', {
                parse_mode: 'HTML',
    ...
    step1() {
        ...
        composer.on(message('text'), async (ctx) => {
            ...
            const item = await this.equipmentItemService.changeItemName({
                id: itemId,
                name: message,
            });
