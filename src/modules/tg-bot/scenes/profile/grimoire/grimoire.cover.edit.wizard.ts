...
@Injectable()
export class GrimoireCoverEditWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_GRIMOIRE_COVER_SCENE_ID,
            this.step1()
        );
        ...
        composer.on(message('document'), async (ctx) => {
            try {
                ...
                const file = await this.downloadFile(
                    url.href,
                    `Assets/images/grimoire/${ctx.from.id}`,
                ...
                this.grimoireService.updateGrimoreImage(ctx.from.id, file);
                this.logger.log(
                    LOGGER_INFO,
                    `🟢 Пользователь успешно изменил фотографию гримуара. * { name: ${ctx.update.message.from.first_name} id: ${ctx.update.message.from.id}}`
                );
                await ctx.scene.enter(ENUM_SCENES_ID.GRIMOIRE_SCENE_ID);
            } catch (e) {
                this.logger.log(
                    LOGGER_EXCEPTION,
                    `🔴  Пользователь не смог изменить фотографию гримуара. * { name: ${ctx.update.message.from.first_name} id: ${ctx.update.message.from.id}}`
                );
                await ctx.scene.enter(ENUM_SCENES_ID.GRIMOIRE_SCENE_ID);
            }
        });
