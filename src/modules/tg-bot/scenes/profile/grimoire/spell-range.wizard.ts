...
export class SpellRangeWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_SPELL_RANGE_SCENE_ID,
            this.step1()
        );
    ...
    start() {
        return async (ctx: BotContext) => {
            await ctx.reply('Область действия заклинаия заклинания.');
        };
    }
    step1() {
        ...
        composer.on(message('text'), async (ctx) => {
            ...
            if (ctx.session.editUnionSpellId) {
                ...
                await this.grimoireService.updateSpellRange(union.spell.id, {
                    range: message,
                });
                await ctx.scene.enter(ENUM_SCENES_ID.ALL_DEVILS_SCENE_ID);
            } else {
                const spellId = ctx.session.spellEdit.spellId;
                await this.grimoireService.updateSpellRange(spellId, {
                    range: message,
                });
