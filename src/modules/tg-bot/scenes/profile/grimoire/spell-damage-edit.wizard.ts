...
@Injectable()
export class SpellDamageEditWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_SPELL_DAMAGE_SCENE_ID,
            this.step1()
        );
    ...
    step1() {
        ...
        composer.on(message('text'), async (ctx) => {
            ...
            if (ctx.session.editUnionSpellId) {
                ...
                await this.grimoireService.updageSpellDamage(
                    union.spell.id,
                    message
                );
                ...
                const spellId = ctx.session.spellEdit.spellId;
                await this.grimoireService.updageSpellDamage(spellId, message);
