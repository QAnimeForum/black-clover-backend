...
@Injectable()
export class SpellGoalsEditWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_SPELL_GOALS_SCENE_ID,
            this.step1()
        );
    ...
    start() {
        return async (ctx: BotContext) => {
            await ctx.reply('Введите новые цели заклинания.');
        };
    }
    step1() {
        ...
        composer.command('cancel', async (ctx) => {
            await ctx.reply('Введите цели заклинания.');
