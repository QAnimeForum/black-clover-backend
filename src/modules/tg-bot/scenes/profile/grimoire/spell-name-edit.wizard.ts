...
@Injectable()
export class SpellNameEditWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_SPELL_NAME_SCENE_ID,
            this.step1()
        );
    ...
    step1() {
        ...
        composer.on(message('text'), async (ctx) => {
            ...
            if (ctx.session.editUnionSpellId) {
                ...
                await this.grimoireService.updateSpellName(union.spell.id, {
                    name: message,
                });
                await ctx.scene.enter(ENUM_SCENES_ID.ALL_DEVILS_SCENE_ID);
            } else {
                const spellId = ctx.session.spellEdit.spellId;
                await this.grimoireService.updateSpellName(spellId, {
                    name: message,
                });
