...
@Injectable()
export class SpellMinimalLevelEditWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_SPELL_MINIMAL_LEVEL_SCENE_ID,
            this.step1()
        );
    ...
    start() {
        return async (ctx: BotContext) => {
            await ctx.reply(
                'Введите минимальный уровень пероснажа для использования заклинания.'
            );
        };
    }
    step1() {
        ...
        composer.command('cancel', async (ctx) => {
            await ctx.reply(
                'минимальный уровень пероснажа для использования заклинания не изменён.'
            );
