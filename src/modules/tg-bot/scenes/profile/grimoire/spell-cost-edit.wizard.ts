...
@Injectable()
export class SpellCostEditWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_SPELL_COST_SCENE_ID,
            this.step1()
        );
    ...
    start() {
        return async (ctx: BotContext) => {
            await ctx.reply('Введите новую стоимость заклинания.');
        };
    }
    step1() {
        ...
        composer.command('cancel', async (ctx) => {
            await ctx.reply('Стоимость заклинаня не изменена.');
        ...
        composer.on(message('text'), async (ctx) => {
            ...
            if (ctx.session.editUnionSpellId) {
                ...
                await this.grimoireService.updateSpellCost(union.spell.id, {
                    cost: message,
                });
                await ctx.scene.enter(ENUM_SCENES_ID.ALL_DEVILS_SCENE_ID);
            } else {
                const spellId = ctx.session.spellEdit.spellId;
                await this.grimoireService.updateSpellCost(spellId, {
                    cost: message,
                });
