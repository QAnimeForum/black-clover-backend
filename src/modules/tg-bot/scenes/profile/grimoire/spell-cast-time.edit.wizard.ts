...
@Injectable()
export class SpellCastTimeEditWiazard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.EDIT_SPELL_CAST_TIME_SCENE_ID,
            this.step1()
        );
    ...
    step1() {
        ...
        composer.on(message('text'), async (ctx) => {
            ...
            if (ctx.session.editUnionSpellId) {
                ...
                await this.grimoireService.updateSpellCastTime(union.spell.id, {
                    castTime: message,
                });
                await ctx.scene.enter(ENUM_SCENES_ID.ALL_DEVILS_SCENE_ID);
            } else {
                const spellId = ctx.session.spellEdit.spellId;
                await this.grimoireService.updateSpellCastTime(spellId, {
                    castTime: message,
                });
