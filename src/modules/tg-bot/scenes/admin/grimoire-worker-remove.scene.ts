...
@Injectable()
export class GrmoireWorkerRemoveWizard {
    ...
    constructor(
        ...
        this.scene = new Scenes.WizardScene<BotContext>(
            ENUM_SCENES_ID.GRIMOIRE_WORKER_REMOVE_SCENE_ID,
            this.step1()
        );
    ...
    start() {
        return async (ctx: BotContext) => {
            await ctx.reply(
                `🧟 Введи ID игрока, которого хотите назначить в башню гримуаров.`
            );
        };
    }
    step1() {
        ...
        composer.on(message('text'), async (ctx) => {
            ...
            const isCharacterWorker =
                await this.grimoireWorkerService.exists(character);
            ...
            try {
                const grimoireWorker =
                    await this.grimoireWorkerService.deleteGrimoireWorker(
                        character.id
                    );
                this.logger.log(
                    LOGGER_INFO,
